...
/*! \public class Lizard extends Entity
    \brief Abstractizeaza notiunea de entitate de tip lizard.
 */
public class Lizard extends Entity
{
    ...
    public Lizard(int id,int x,int y)
    {
        super(Assets.lizard[0], id);
                VelX=2;
                facing=0;
                ENTITY_HEIGHT=24;
                ENTITY_WIDTH=32;
    ...
    public void Draw(Graphics g, int x, int y)
    {
        /// daca entitatea inca nu este desenat se va desena la pozitia din harta, altfel este desenat in functie de valorile EntityX si EntityY
            if (EntityX == 0 && EntityY == 0 && start == true) {
                ...
                EntityY = y + 24;
            if (facing == 0) {
                g.drawImage(Assets.lizard[frame + 6], EntityX, EntityY, ENTITY_WIDTH, ENTITY_HEIGHT, null);
            } else if (facing == 1)
                g.drawImage(Assets.lizard[frame], EntityX, EntityY, ENTITY_WIDTH, ENTITY_HEIGHT, null);
    }
    @Override
    public void Update() {
...
*/
    ...
    {
        /// se verifica daca mobul se afla in aer si se intoarce pt a nu cadea sau daca atinge un perete
        int air=1;
